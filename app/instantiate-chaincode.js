...
const instantiateChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion,chaincodeType,functionName, args, username, org_name) {
	...
	try {
		...
		logger.debug('Successfully get  the deploy id  "%s"',deployId);
		...
		const request = {
			...
			'endorsement-policy': {
			        identities: [
					{ role: { name: 'member', mspId: 'Bank1MSP' }},
					{ role: { name: 'member', mspId: 'Bank2MSP' }}
			        ],
			        policy: {
		...
		let results = await channel.sendInstantiateProposal(request, 220000); //instantiate takes much longer
		...
		logger.debug("all good is : %s",all_good);
		if (all_good) {
